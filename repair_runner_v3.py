import json, os, subprocess, uuid, shutil, hashlib, sys
...
def run_smoke():
    ...
    # Hide global skill so we ensure the plugin is self-sufficient
    ...
    if session_dir.exists() and (session_dir / "evidence.jsonl").exists():
        for line in open(session_dir / "evidence.jsonl"):
            if not line.strip(): continue
            if '"source": "read"' in line: post += 1
    
    # Since write was denied, pre_tool fired at least once
    # Check ndjson for blocked tool
    blocked = False
    with open(ndjson_out) as f:
...
def check_contamination(config):
    ...
    if config == "BASELINE":
        return not global_skill and not plugin_enabled
    elif config == "GRAVITAS_CORE":
        return global_skill and not plugin_enabled
    elif config == "GRAVITAS_NATIVE":
        return not global_skill and plugin_enabled
    return False
...
def parse_ndjson(path):
                ...
                if data.get("event") == "init":
                    ...
                    if su.get("step_type") == "tool" and su.get("state") == "DONE":
                        ...
                        if tn == "view_file" or tn == "grep_search": 
                            res["reads"] += 1
                            if tn == "view_file" and "AbsolutePath" in ti.get("parameters", {}):
                                files_read.append(ti["parameters"]["AbsolutePath"])
    ...
    if len(files_read) > 0:
        res["duplicate_read_ratio"] = round(1.0 - (unique / len(files_read)), 2)
    return res
...
def get_hook_evidence(conv_id):
    ...
    # Assuming pre_tool fired if there are tools
...
def run_task():
    ...
    for cfg in configs:
        ...
        # Parse Task A manifest
        task_yaml = Path("fixtures/L1_simple_bug/manifest.yaml")
        repo_url = ""
        ...
        # For Native, pre_tool count must be >= tools (read + writes)
    ...
    isolate_config("GRAVITAS_CORE", env) # Back to normal user state
    
    Path("repaired-canary-v3.json").write_text(json.dumps(results, indent=2))
    print("Done. Wrote repaired-canary-v3.json")
